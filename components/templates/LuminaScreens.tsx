...
/**
 * Превью для карточки в каталоге. Там сетка на три колонки, поэтому
 * колонка уже, чем на странице шаблона, и коэффициент меньше.
 *
 * Чуть меньше, чем нужно на самом деле: обрезка снизу читается как
 * «страница продолжается», а тёмные поля по краям — как окно браузера.
 * Обратное (масштаб с запасом) выглядело бы обрезанным по бокам.
 */
 ...
 *
 * Раньше все карточки рисовались одной обобщённой CSS-схемой
 * (TemplatePreview): шапка, две полоски заголовка, сетка карточек. От
 *личались они только цветом, поэтому шаблоны выглядели одинаковыми и
 * каталог не продавал. Теперь у проработанного шаблона в карточке стоит
 * его собственный первый экран — тот же компонент, что и в предпросмотре.
 */
export function LuminaCardPreview({ className = "" }: { className?: string }) {
  return (
    <div
      className={`relative aspect-[16/10] w-full overflow-hidden bg-[#17111c] text-white ${className}`}
    >
      <div
