import { CASES, SERVICES } from "./law/data";
import { Cases, Hero, Request, Services } from "./law/sections";

/**
 * Экраны шаблона на странице `/templates/law-office` — «скриншоты».
 *
 * Рендерятся теми же секциями, что и живой предпросмотр, просто
 * уменьшенными. Раньше такие макеты рисовались вручную, и они разошлись
 * с предпросмотром: правился один — второй устаревал. Здесь расхождение
 ...
 * в колонку 480 px, вёрстка останется десктопной, а текст окажется втрое
 * крупнее — получится не скриншот, а увеличенный фрагмент. Поэтому секция
 * рендерится в натуральную ширину 1024 px, а `zoom` уменьшает её вместе
 * с занимаемым местом.
 *
 ...
 */
export function LawCardPreview({ className = "" }: { className?: string }) {
  return (
    <div
      className={`relative aspect-[16/10] w-full overflow-hidden bg-[#1e1b33] text-white ${className}`}
    >
      <div
...
/**
 * Рамка одного экрана. Тёмная база совпадает с фоном шаблона (#1e1b33),
...
function Frame({
  ...
  return (
    <div className="overflow-hidden rounded-2xl bg-[#1e1b33] text-white">
      <Chrome label={label} />
...
function Fade() {
  return (
    <div className="pointer-events-none absolute inset-x-0 bottom-0 h-16 bg-gradient-to-t from-[#1e1b33] to-transparent" />
  );
}

type Variant = "hero" | "services" | "cases" | "request";
 ...
 */
export function LawScreen({ variant }: { variant: Variant }) {
  if (variant === "hero")
    return (
  ...
  if (variant === "services")
    return (
      <div className="relative">
        <Frame label="вашсайт.рф#services" maxH="20rem">
          <Services cat="all" shown={SERVICES} />
        </Frame>
        <Fade />
      </div>
    );
  ...
  if (variant === "cases")
    return (
      <div className="relative">
        <Frame label="вашсайт.рф#cases" maxH="20rem">
          <Cases cases={CASES} />
        </Frame>
        <Fade />
      </div>
    ...
    <div className="relative">
      <Frame label="вашсайт.рф#request" maxH="24rem">
        <Request
          ...
          slots={["Утром, 9:00 – 12:00", "Днём, 12:00 – 17:00", "Вечером, после 18:00"]}
        />
      </Frame>
      <Fade />
    </div>
